...
model_parameters <- list(
  # human
  bh = 1/(60*365), # birthrate; from paper
  m = 2, # initial ratio of mosquitoes to humans; from literature
  c = 0.5, # contact rate (biting rate); from paper
  p_h = 0.38, # probability of a human being infected by an infected mosquito bite; paper 0.38
  ...
  mu_h = 1/(60*365), # human death rate; set equal to birthrate
  p_IIP = 1/5, # progression rate out of human E state; from paper
  p_IP = 1/6, # progression rate out of human I state; from paper
...
beta_h <- julia_eval("beta_h = c * p_h * ((0.3 *(cos.(((2*pi*tspan).+ 5.295594)/365))).+ 1);") #seasonal transmission- mosquito to human
...
E_init <- matrix(0, nrow=2, ncol=n_stype)
I_init <- matrix(100, nrow=2, ncol=n_stype)
S_naive_init <- round(initial_s * pop) - sum(I_init) - sum(E_init)
S_second_init <- round(initial_s2 * pop)
R_prim_init <- round(initial_r1 * pop)
R_second_init <- round(rep((initial_r2 * pop)/n_stype, n_stype))
...
julia_assign('u0Eh_12', E_init[2, 1])
julia_assign('u0Eh_21', E_init[2, 2])
...
julia_assign("u0Rh_12", R_second_init[1])
julia_assign("u0Rh_21", R_second_init[2])
...
am_pop <- Smdt + E1mdt + E2mdt + I1mdt + I2mdt
