...
julia_source("1_dengue_4st.jl")
#x <- julia_eval("dengue_4st!(u0[1], par, 2, 7)")
julia_source("2_run_4st_model.jl")

julia_exists("dengue_4st!") # test whether functions loaded and exist :)
julia_exists("multinom_samp")
julia_exists("run_4st_model!")
...
result <- julia_eval("run_4st_model!(nsims, tmax, u0, outcomes0, par, dt)")
...
plot_humans_by_seroptype <- function() {
  ...
  for (i in 1:nsims) {
    ...
    lines(result$time, df3[, i], col = alpha(colors[3], 0.3))
    lines(result$time, df4[, i], col = alpha(colors[4], 0.3))
  }
}
...
#      ylim = c(1, max(df1, df2, df3, df4)), 
      ylab = this_lab, xlab = "Days", bty = "n")#, log='y')
  for (i in 1:plot_nsims) {
    lines(result$time, df1[, i], col = alpha(colors[1], sim_alpha))
    lines(result$time, df2[, i], col = alpha(colors[2], sim_alpha))
    lines(result$time, df3[, i], col = alpha(colors[3], sim_alpha))
...
plot_populations <- function() {
  ...
  df$median <- apply(df, 1, median)
  plot(NA, NA, xlim = c(0, tmax), ylim = c(0, max(df)), ylab = "Human pop", xlab = "Days", bty = "n")
  for (i in 1:nsims) {
    lines(result$time, df[, i], col = alpha(colors[4], 0.15))
  }
  lines(result$time, df$median, col = colors[4])
  ...
  df$median <- apply(df, 1, median)
  plot(NA, NA, xlim = c(0, tmax), ylim = c(0, max(df)), ylab = "Mosquito pop", xlab = "Days", bty = "n")
  for (i in 1:nsims) {
    lines(result$time, df[, i], col = alpha(colors[4], 0.15))
  }
  lines(result$time, df$median, col = colors[4])
