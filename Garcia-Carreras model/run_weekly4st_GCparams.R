...
#x <- julia_eval("dengue_4st_imp!(u0[1], par, 2)")
...
model_parameters <- list(
  # human
  bh = 3.55e-5 * 7, #1/(60*365) * 7, # birthrate; from paper
  mu_h = 3.55e-5 * 7, #1/(60*365) * 7, # human death rate; set equal to birthrate
  ...
  p_IIP = 1/5.9 * 7, # progression rate out of human E state; from paper
  p_IP = 1/5 * 7, # progression rate out of human I state; from paper
  ...
  weekly_import = 1.5 * 4 # total across serotypes
)
...
TempFunSine = function(tp, t_amp = 5, t_mean = 28, t_seas = 0) {
  ...
  return(t_amp / 2 * sin(2 * pi * tp / 52 - t_s) + t_mean)
}
tmp = TempFunSine(tspan)
biterate = 7 * Vectorize(BiteRate, 'tmp')(tmp, sp = species)  # = 0.2492967
...
beta_h_orig <- julia_eval("beta_h_orig = c * p_h * ((0.3 *(cos.(((2*pi*tspan).+ 5.295594)/52))).+ 1);") #seasonal transmission- mosquito to human
...
beta_m_orig <- julia_eval("beta_m_orig = c * p_m;") # human-to-mosquito transmission rate
#julia_assign('beta_m', rep(beta_m_orig, length(beta_h)))

### mosquito population dynamics
# mortrate = Vectorize(MortRate, 'tmp')(tmp, sp = species)  # = 0.034206
# incurate = Vectorize(IncuRate, 'tmp')(tmp, sp = species)  # = 0.1181627
# eggslaid = Vectorize(EggsLaid, 'tmp')(tmp, sp = species)  # = 7.45787
# survprob = Vectorize(SurvProb, 'tmp')(tmp, sp = species)  # = 0.9157967
# devlrate = Vectorize(DevlRate, 'tmp')(tmp, sp = species)  # = 0.108575
# carrcapa = Vectorize(CarrCap, 'tmp')(tmp = tmp, N_max = pop * m, sp = species)  # = 188464.8
# birth_rate = eggslaid * survprob * devlrate * N_v * (1 - N_v / carrcapa) / mortrate 
...
julia_command("par = (; bh, beta_h, bm, phi_m, beta_m, mu_m, mu_mL, mu_h, p_IIP, p_IP, p_EIP, p_R, m, weekly_import)") 
...
for (i in 1:nsims) {
  ...
  #  sample.int(4)
  init_compartments[c('Ih_1', 'Ih_2', 'Ih_3', 'Ih_4'), i] <- 
    10 * sample.int(4)
}
...
plot_I_by_seroptype <- function(plot_spp = 'humans', plot_nsims = NULL) {
  if (plot_spp == 'humans') {
    df1 <- as.data.frame(result$Ih_1 + result$Ih_21 + result$Ih_31 + result$Ih_41)
    df2 <- as.data.frame(result$Ih_2 + result$Ih_12 + result$Ih_32 + result$Ih_42)
    df3 <- as.data.frame(result$Ih_3 + result$Ih_13 + result$Ih_23 + result$Ih_43)
    df4 <- as.data.frame(result$Ih_4 + result$Ih_14 + result$Ih_24 + result$Ih_34)
  } else {
    df1 <- as.data.frame(result$Im1)
    df2 <- as.data.frame(result$Im2)
    df3 <- as.data.frame(result$Im3)
    df4 <- as.data.frame(result$Im4)
  }
  if (is.null(plot_nsims)) plot_nsims <- ncol(df1)
  
  sim_alpha <- ifelse(plot_nsims == 1, 1, 0.3)
  this_lab <- paste('Infectious', plot_spp)
  plot(NA, NA, 
#      xlim = c(0, 20), 
      xlim = c(300, tmax), 
      ylim = c(0, 800), 
#      ylim = c(1, max(df1, df2, df3, df4)), 
      ylab = this_lab, xlab = "Weeks", bty = "n")#, log='y')
  for (i in 1:plot_nsims) {
    lines(1:tmax, df1[, i], col = alpha(colors[1], sim_alpha))
    lines(1:tmax, df2[, i], col = alpha(colors[2], sim_alpha))
    lines(1:tmax, df3[, i], col = alpha(colors[3], sim_alpha))
    lines(1:tmax, df4[, i], col = alpha(colors[4], sim_alpha))
  }
}
plot_I_by_seroptype()
